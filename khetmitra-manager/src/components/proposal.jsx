...
const sendRequest = async (senderName, recipientEmail, proposalDetails) => {
  const mailPromise = fetch("/api/sendmail", {
    method: "POST",
...
const Proposal=({ name , onClose})=>{
    ...
    const getCropRecommendation = async () => {
        ...
        try {
            // Use soil parameters from the name prop
            const cropParams = {
            ...
            toast.error("Error getting crop recommendation");
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch("/api/add_prosal", {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
