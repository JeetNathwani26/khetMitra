    ...
    fetch("http://127.0.0.1:5000/verify", {
     ...
     credentials: "include", // ✅ important to send session cookie
      ...
      .then((data) => {
        ...
        // After setting email, fetch user data and navigate accordingly
        fetch("http://localhost:5000/get_data")
          .then((res) => res.json())
          .then((users) => {
            ...
            const matchedUsers = users.filter((u) => u.email.toLowerCase() === userEmail.toLowerCase());
            if (matchedUsers.length === 0) {
              // No user found, navigate to Register page
              navigate("/Register");
            } else {
              ...
              } else if (matchedUsers[0].role === 'Manager') {
                navigate("/landowner_home");
...
const sign = async (e) => {
  ...
  try {
    const res = await fetch("http://localhost:5000/gets_data", {
      method: "POST",               
      credentials: "include",       
      ...
      <div className="h- md:h-[90vh] w-full flex justify-center items-center p-2 ">
        ...
        >
          ...
          <div className=" w-30 scale-73 md:w-40 md:scale-90 p-2 origin-top-left ">
            <ReCAPTCHA
              sitekey="6LcYIeYrAAAAACrnr_WlUPlS0V85AybnREFcbnou"
          ...
          <div className="items-center text-center">
            ...
            <GoogleOAuthProvider clientId="656820879847-4qtalivq7nitn908s0ossdatvecf8df6.apps.googleusercontent.com">
              <GoogleLogin 
                onSuccess={handl} 
