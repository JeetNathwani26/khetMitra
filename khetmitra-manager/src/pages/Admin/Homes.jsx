...
const block = async (item,text) => {
  const mailsend=fetch("/api/sendmail", {
    method: "POST",
    headers: {
   ...
   const  text=`
        <div style="font-family: Arial, sans-serif; color: #333; text-align: center;">
          ...
          <p style="font-size: 16px;">
            Your manager account has been <b style="color:red;">BLOCKED 🚫</b> by the admin.<br/>
...
useEffect(() => {
    ...
    fetch("/api/get_data")
      ...
      .then((data) => {
        ...
        const mdata = data.filter(
          (u) => u.role === "Manager" && u.status?.toLowerCase() !== "pendding"
        );
        console.log(mdata);
        setUsers(mdata);
      })
  ...
  const pdata=fdata.filter((u) => u.role === "Manager" && u.status ==="pendding"); 
  const links = [
    { name: `Manager(${users.length})`, path: "/manager" },
    { name: `Pending(${pdata.length})`, path: "/mpending" },
  ];
              ...
              <table className="w-full">
                <tbody>
                  {displayList.map((s, index) => (
                    <tr key={index} className="border-0 shadow-sm hover:bg-gray-100 backdrop-blur-md">
